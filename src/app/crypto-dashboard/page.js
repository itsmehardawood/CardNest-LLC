...
// ── Crypto-specific screens ──
import CryptoSidebar from "../components/Crypto-Dashboard-Screens/CryptoSidebar";
import CryptoHomeScreen from "../components/Crypto-Dashboard-Screens/CryptoHomeScreen";
import CryptoValidationScreen from "../components/Crypto-Dashboard-Screens/CryptoValidationScreen";
import AddressScreeningScreen from "../components/Crypto-Dashboard-Screens/AddressScreeningScreen";
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-4 text-gray-600">Loading crypto dashboard...</p>
      </div>
    </div>
  );
}
...
function CryptoDashboardContent() {
  ...
  // Business profile form state (reused for profile screen)
  ...
  // ── Business profile handlers (shared) ──
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const isSuccess = result.status === true || result.success === true || result.success === "true" ||
          result.message?.includes("successfully") || result.status === "success" || response.status === 200 || response.status === 201;
  ...
  const sidebarItems = [
    ...
    { id: "crypto-validation", label: "Crypto Validation" },
    { id: "address-screening", label: "Address Screening" },
  ...
  const getHeaderTitle = () => {
    ...
    return sidebarItems.find((i) => i.id === activeTab)?.label || "Crypto Dashboard";
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "home":
        return <CryptoHomeScreen status={status} setActiveTab={handleTabChange} />;
