...
import { FiCheckCircle, FiClock, FiAlertCircle, FiDownload, FiTrash2, FiUpload, FiFileText, FiCopy, FiExternalLink } from 'react-icons/fi';
...
function DocumentsScreen({ documents, setActiveTab, handleFileUpload }) {
        ...
        if (storedUser) {
          ...
          if (userObj.id) {
            ...
            if (response.ok) {
              ...
              setApiError('Failed to load verification status. Please try again later.');
            }
          }
        }
      } catch (error) {
  ...
  const handleDownload = (docUrl, docName) => {
    ...
    try {
      ...
      if (!docUrl.startsWith('https://')) {
        console.error('Insecure document URL');
        return;
      }
  ...
  const handleDelete = (docName, index) => {
    if (window.confirm(`Are you sure you want to permanently delete ${docName}? This action cannot be undone.`)) {
  ...
  const handleDirectUpload = (e) => {
    ...
    for (let i = 0; i < e.target.files.length; i++) {
      ...
      if (e.target.files[i].size > 10 * 1024 * 1024) { // 10MB limit
  ...
  const getFileIcon = (fileName) => {
    ...
    switch (extension) {
      ...
      case 'jpg': case 'jpeg': case 'png': return <FiFileText className="text-blue-500 text-xl" />;
      case 'doc': case 'docx': return <FiFileText className="text-indigo-500 text-xl" />;
      default: return <FiFileText className="text-gray-500 text-xl" />;
    }
  };
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mt-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-800">API Credentials</h3>
          ...
          {userData?.merchant_id && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded">Public</span>
              </div>
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 px-4 py-2 text-sm font-mono bg-white border rounded-lg break-all">
                  {userData.merchant_id}
                </div>
                <button
          ...
          {aesKey && (
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <p className="text-xs text-gray-500 mt-2">
                <strong>Security Notice:</strong> This key grants full API access. Never share it or expose it in client-side code.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-gray-800">Verified Documents</h2>
          ...
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <p className="text-green-700 text-sm">
              Your business documents have been successfully verified and your account is fully active.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            {profile?.registration_document_path && (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center">
                    <FiFileText className="text-xl" />
                  </div>
                  <div>
                    <p className="font-medium text-gray-800">Business Registration Document</p>
                    <p className="text-sm text-gray-600">Required Document</p>
                    <p className="text-xs text-gray-500">
                      Verified on {new Date(profile?.updated_at).toLocaleDateString('en-US', { 
                        year: 'numeric', 
                        month: 'long', 
                        day: 'numeric' 
                      })}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button 
                    onClick={() => handleDownload(profile?.registration_document_path, 'Business_Registration_Document')}
                    className="px-3 py-1 text-blue-600 hover:text-blue-800 text-sm hover:bg-blue-50 rounded flex items-center gap-1 transition-colors"
          ...
          <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <h4 className="font-medium text-gray-800 mb-3">Verified Business Information</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-600">Business Name</p>
                <p className="font-medium text-gray-800 break-words">{profile?.business_name}</p>
              </div>
              <div>
                <p className="text-gray-600">Registration Number</p>
                <p className="font-medium text-gray-800 break-words">{profile?.business_registration_number}</p>
              </div>
              <div>
                <p className="text-gray-600">Account Holder</p>
                <p className="font-medium text-gray-800 break-words">{profile?.account_holder_first_name} {profile?.account_holder_last_name}</p>
              </div>
              <div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <h3 className="text-lg font-semibold text-gray-800 mb-2">Security Reminder</h3>
                  <button 
                ...
                <p className="text-sm text-gray-600 mb-3">
                  Your documents contain sensitive information. Please ensure you:
                </p>
                <ul className="text-sm text-gray-700 space-y-2">
                  <li className="flex items-start gap-2">
                    <span>•</span>
                    <span>Never share your documents or credentials via unsecured channels</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span>•</span>
                    <span>Only download documents on secure, private networks</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span>•</span>
                    <span>Regularly review and update your uploaded documents</span>
                  </li>
                </ul>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-800">Documents Under Review</h2>
        ...
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <p className="text-yellow-700 text-sm">
            ...
            This process typically takes 1-3 business days. You will receive a notification once completed.
          </p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Required Document</p>
                  <p className="text-xs text-gray-500">
                    Submitted on {new Date(profile?.created_at).toLocaleDateString('en-US', { 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
                    })}
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h4 className="font-medium text-gray-800 mb-3">Submitted Business Information</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-gray-600">Business Name</p>
              <p className="font-medium text-gray-800 break-words">{profile?.business_name}</p>
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-medium text-blue-800 mb-1">Need to update your documents?</h4>
              <p className="text-blue-700 text-sm">
                If you need to submit updated documents, please contact our support team.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Upload and manage your business verification documents</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button 
              ...
              className="px-4 py-2 bg-white border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 text-sm transition-colors flex items-center justify-center gap-2"
        ...
        {documents.length > 0 ? (
          <div className="space-y-3">
            {documents.map((doc, index) => (
              <div key={index} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 gap-4">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="px-3 py-1 text-blue-600 hover:text-blue-800 text-sm hover:bg-blue-50 rounded transition-colors flex items-center gap-1"
                  ...
                  <button 
                    ...
                    className="px-3 py-1 text-red-600 hover:text-red-800 text-sm hover:bg-red-50 rounded transition-colors flex items-center gap-1"
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 text-sm mb-6 px-4 max-w-md mx-auto">
              Upload your business registration documents to complete your profile verification process and access all features.
            </p>
            <div className="flex flex-col sm:flex-row justify-center gap-3">
              ...
              {handleFileUpload && (
                <label className="px-4 py-2 border border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 text-sm cursor-pointer transition-colors flex items-center justify-center gap-2">
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            ...
            <ul className="text-blue-700 text-sm space-y-2">
              <li className="flex items-start gap-2">
                <span>•</span>
                <span>Business registration document is required for verification</span>
              </li>
              <li className="flex items-start gap-2">
                <span>•</span>
                <span>Accepted formats: PDF, DOC, DOCX, JPG, PNG (max 10MB each)</span>
              </li>
              <li className="flex items-start gap-2">
                <span>•</span>
                <span>Documents must be valid, unexpired, and clearly readable</span>
              </li>
              <li className="flex items-start gap-2">
                <span>•</span>
                <span>Ensure all corners and text are visible in photos/scans</span>
              </li>
            </ul>
          </div>
        </div>
          ...
          <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-yellow-700 text-sm">
                  All uploaded documents are encrypted and stored securely. Only authorized personnel can access them for verification purposes.
                </p>
              </div>
            </div>
          </div>
