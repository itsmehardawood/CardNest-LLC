...
/**
 * Sidebar for the Crypto Validation dashboard.
 * Mirrors the visual style of the Card-Scan sidebar but with crypto-specific menu items.
 */
const CryptoSidebar = ({ sidebarOpen, setSidebarOpen, activeTab, setActiveTab, status }) => {
  ...
  const baseSidebarItems = [
    ...
    // { id: 'crypto-validation', label: 'Crypto Validation', icon: <ShieldCheck className="w-5 h-5" /> },
    // { id: 'address-screening', label: 'Address Screening', icon: <Search className="w-5 h-5" /> },
      ...
      <div className="flex items-center justify-between p-3.5 border-b border-gray-700">
        {sidebarOpen && (
          <div className="flex items-center space-x-2">
            <Coins className="w-5 h-5" style={{ color: '#e0aa3e' }} />
