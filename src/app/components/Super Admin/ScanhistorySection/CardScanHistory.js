...
//     if (successRate >= 95) {
//       return <span className="px-2 py-1 text-xs font-medium bg-green-900 text-green-300 rounded-full">Excellent</span>;
//     } else if (successRate >= 80) {
//       return <span className="px-2 py-1 text-xs font-medium bg-yellow-900 text-yellow-300 rounded-full">Good</span>;
//     } else {
//       return <span className="px-2 py-1 text-xs font-medium bg-red-900 text-red-300 rounded-full">Needs Attention</span>;
//     }
//   };

//   return (
//     <div className="bg-black shadow-sm rounded-lg overflow-hidden border border-gray-800">
...
//                 className="border border-gray-700 rounded-lg p-4 hover:border-blue-500 hover:shadow-md cursor-pointer transition-all duration-200"
//               >
...
//                     <div className="font-medium text-white">{merchant.unique_users}</div>
//                     <div className="text-gray-400">Unique Users</div>
//                   </div>
//                   <div className="text-center">
//                     <div className="font-medium text-white">{merchant.unique_cards}</div>
//                     <div className="text-gray-400">Unique Cards</div>
//                   </div>
//                   <div className="text-center">
//                     <div className="font-medium text-green-400">{merchant.success_rate}%</div>
...
//     <div className="p-6">
//       <div className="mb-6">
//         <h1 className="text-2xl font-bold text-white">Card Scan History</h1>
//         <p className="text-gray-300 mt-1">View and manage card scan records by merchant</p>
//       </div>
...
const MerchantList = ({ scanData, onMerchantClick, searchQuery }) => {
  ...
  const getStatusBadge = (successRate) => {
    if (successRate >= 95) {
      return <span className="px-2 py-1 text-xs font-medium bg-green-900/50 text-green-300 rounded-full backdrop-blur-sm">Excellent</span>;
    } else if (successRate >= 80) {
      return <span className="px-2 py-1 text-xs font-medium bg-yellow-900/50 text-yellow-300 rounded-full backdrop-blur-sm">Good</span>;
    } else {
      return <span className="px-2 py-1 text-xs font-medium bg-red-900/50 text-red-300 rounded-full backdrop-blur-sm">Needs Attention</span>;
    }
  };
  ...
  return (
    <div className="bg-black/50 backdrop-blur-md shadow-2xl rounded-2xl overflow-hidden border border-gray-700/50 p-8">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-gray-100">Merchant Performance</h2>
        <p className="text-sm text-gray-400 mt-1">Overview of scan activities across all merchants</p>
      </div>
      <div>
        {merchantStats.length === 0 ? (
          <div className="text-center py-12 text-gray-400 bg-black/30 rounded-lg backdrop-blur-sm">
            ...
            <p className="mt-4 text-lg font-medium text-gray-300">{searchQuery ? 'No results found for search' : 'No scan data available'}</p>
            <p className="mt-1 text-sm text-gray-400">{searchQuery ? 'Try adjusting your search terms' : 'Scan data will appear here once available'}</p>
          </div>
        ) : (
            ...
            {merchantStats.map((merchant) => (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-3xl font-bold text-blue-400">{merchant.total_scans}</div>
                    <div className="text-sm text-gray-400 uppercase tracking-wide font-medium">Total Scans</div>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-gray-700/50 flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-sm text-gray-400">
                      Success Rate: <span className="font-medium text-green-400">{merchant.success_rate}%</span>
                    </div>
                  ...
                  <div className="flex items-center space-x-6 text-sm">
                    <div className="text-center">
                      <div className="font-medium text-gray-100">{merchant.unique_users}</div>
                      <div className="text-xs text-gray-400">Users</div>
                    </div>
                    <div className="w-px h-6 bg-gray-700/50"></div>
                    <div className="text-center">
                      <div className="font-medium text-gray-100">{merchant.unique_cards}</div>
...
const StatsOverview = ({ scanData }) => {
  const stats = useMemo(() => {
    ...
    const uniqueCards = new Set(scanData.map(scan => scan.card_number_masked)).size;
    const successCount = scanData.filter(scan => scan.status === 'success').length;
      ...
      <div className="bg-black/30 backdrop-blur-md rounded-2xl p-6 border border-gray-700/50 hover:shadow-xl transition-all duration-300">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-gray-100">{stats.totalScans}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 backdrop-blur-md rounded-2xl p-6 border border-gray-700/50 hover:shadow-xl transition-all duration-300">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-gray-100">{stats.uniqueUsers}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 backdrop-blur-md rounded-2xl p-6 border border-gray-700/50 hover:shadow-xl transition-all duration-300">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-gray-100">{stats.uniqueCards}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-black/30 backdrop-blur-md rounded-2xl p-6 border border-gray-700/50 hover:shadow-xl transition-all duration-300">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-3xl font-bold text-gray-100">{stats.successRate}%</p>
          </div>
        </div>
      </div>
    </div>
